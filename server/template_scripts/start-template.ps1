...
# Get the parent directory of the EZScripts directory
$ServerRoot = Split-Path -Parent $PSScriptRoot

# Acquire variable hashtable from variables.txt
$ExternalVariablesFile = Join-Path $ServerRoot "variables.txt"
...
Function global:SetupForge
{
    ...
    else
    {
        ...
        DeleteFileSilently  (Join-Path $ServerRoot 'user_jvm_args.txt')
        ...
        "# Xmx and Xms set the maximum and minimum RAM usage, respectively.`n" +
                ...
                "# -Xmx4G`n" +
                "${script:JavaArgs}" | Out-File (Join-Path $ServerRoot 'user_jvm_args.txt') -encoding utf8
    }

    if ((DownloadIfNotExists (Join-Path $ServerRoot $ForgeJarLocation) (Join-Path $ServerRoot 'forge-installer.jar') "${ForgeInstallerUrl}"))
    {
        "Forge Installer downloaded. Installing..."
        RunJavaCommand "-jar (Join-Path $ServerRoot 'forge-installer.jar') --installServer"

        if ([int]$MINOR[1] -gt 16)
        {
            DeleteFileSilently  (Join-Path $ServerRoot 'run.bat')
            DeleteFileSilently  (Join-Path $ServerRoot 'run.sh')
        }
        else
        {
            "Renaming forge-${MinecraftVersion}-${ModLoaderVersion}.jar to forge.jar"
            Move-Item (Join-Path $ServerRoot "forge-${MinecraftVersion}-${ModLoaderVersion}.jar") (Join-Path $ServerRoot 'forge.jar')
            Move-Item (Join-Path $ServerRoot "forge-${MinecraftVersion}-${ModLoaderVersion}-universal.jar") (Join-Path $ServerRoot 'forge.jar')
        }

        if ((Test-Path -Path (Join-Path $ServerRoot $ForgeJarLocation) -PathType Leaf))
        {
            DeleteFileSilently  (Join-Path $ServerRoot 'forge-installer.jar')
            DeleteFileSilently  (Join-Path $ServerRoot 'forge-installer.jar.log')
...
Function global:SetupNeoForge
{
    ...
    if ((DownloadIfNotExists (Join-Path $ServerRoot $ForgeJarLocation) (Join-Path $ServerRoot 'neoforge-installer.jar') "${ForgeInstallerUrl}"))
    {
        "NeoForge Installer downloaded. Installing..."
        RunJavaCommand "-jar (Join-Path $ServerRoot 'neoforge-installer.jar') --installServer"
        ...
        if ((Test-Path -Path (Join-Path $ServerRoot $ForgeJarLocation) -PathType Leaf))
        {
            DeleteFileSilently  (Join-Path $ServerRoot 'neoforge-installer.jar')
            DeleteFileSilently  (Join-Path $ServerRoot 'neoforge-installer.jar.log')
            "Installation complete. forge-installer.jar deleted."
        }
        else
        {
...
Function global:SetupFabric
{
    ...
    if ("${ImprovedFabricLauncherAvailable}" -eq "200")
    {
        ...
        (DownloadIfNotExists (Join-Path $ServerRoot $script:LauncherJarLocation) (Join-Path $ServerRoot $script:LauncherJarLocation) "${ImprovedFabricLauncherUrl}") > $null
    }
    else
    {
        try
        ...
        if ((DownloadIfNotExists (Join-Path $ServerRoot 'fabric-server-launch.jar') (Join-Path $ServerRoot 'fabric-installer.jar') "${FabricInstallerUrl}"))
        {
            "Installer downloaded..."
            $script:LauncherJarLocation = "fabric-server-launch.jar"
            $script:MinecraftServerJarLocation = "server.jar"
            RunJavaCommand "-jar (Join-Path $ServerRoot 'fabric-installer.jar') server -mcversion ${MinecraftVersion} -loader ${ModLoaderVersion} -downloadMinecraft"

            if ((Test-Path -Path (Join-Path $ServerRoot 'fabric-server-launch.jar') -PathType Leaf))
            {
                DeleteFileSilently (Join-Path $ServerRoot '.fabric-installer') -Recurse
                DeleteFileSilently (Join-Path $ServerRoot 'fabric-installer.jar')
...
Function global:SetupQuilt
{
    ...
    elseif ((DownloadIfNotExists (Join-Path $ServerRoot 'quilt-server-launch.jar') (Join-Path $ServerRoot 'quilt-installer.jar') "${QuiltInstallerUrl}"))
    {
        "Installer downloaded. Installing..."
        RunJavaCommand "-jar (Join-Path $ServerRoot 'quilt-installer.jar') install server ${MinecraftVersion} --download-server --install-dir=."

        if ((Test-Path -Path (Join-Path $ServerRoot 'quilt-server-launch.jar') -PathType Leaf))
        {
            DeleteFileSilently (Join-Path $ServerRoot 'quilt-installer.jar')
...
Function global:SetupLegacyFabric
{
    ...
    elseif ((DownloadIfNotExists (Join-Path $ServerRoot 'fabric-server-launch.jar') (Join-Path $ServerRoot 'legacyfabric-installer.jar') "${LegacyFabricInstallerUrl}"))
    {
        "Installer downloaded. Installing..."
        RunJavaCommand "-jar (Join-Path $ServerRoot 'legacyfabric-installer.jar') server -mcversion ${MinecraftVersion} -loader ${ModLoaderVersion} -downloadMinecraft"

        if ((Test-Path -Path (Join-Path $ServerRoot 'fabric-server-launch.jar') -PathType Leaf))
        {
            DeleteFileSilently (Join-Path $ServerRoot 'legacyfabric-installer.jar')
...
Function global:Minecraft
{
    ...
    {
        (DownloadIfNotExists (Join-Path $ServerRoot $MinecraftServerJarLocation) (Join-Path $ServerRoot $MinecraftServerJarLocation) "${MinecraftServerUrl}") > $null
    }
}
...
Function Eula
{
    if (!(Test-Path -Path (Join-Path $ServerRoot 'eula.txt') -PathType Leaf))
    {
        ...
        if (${Answer} -eq "I agree")
        {
            ...
            "#By changing the setting below to TRUE you are indicating your agreement to our EULA (https://aka.ms/MinecraftEULA).`n" +
                    "eula=true" | Out-File (Join-Path $ServerRoot 'eula.txt') -encoding utf8
        }
        else
        {
