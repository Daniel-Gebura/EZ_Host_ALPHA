...
# Command file to check for external commands
$CommandFile = "server-command.txt"

# Clear existing command file if any
if (Test-Path -Path $CommandFile) {
...
$serverProcess = RunJavaCommand "${ServerRunCommand}"

# Create a StreamWriter to send commands to the server process
$pipeWriter = $serverProcess.StandardInput

# Start a separate job to log process output
Start-Job -ScriptBlock { param($p) LogProcessOutput $p } -ArgumentList $serverProcess | Out-Null

# Monitor the command file for new commands
while (-not $serverProcess.HasExited) {
    if (Test-Path -Path $CommandFile) {
        $command = Get-Content -Path $CommandFile -Raw
